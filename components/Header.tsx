                    ...
                    <nav className="hidden md:flex items-center space-x-8">
                        {['HOME', 'FEATURES', 'PREVIEW', 'CONTACT'].map((item) => (
                            <a
                                key={item}
                                href={href(item)}
                                className="text-xs text-white hover:text-zinc-300 transition-colors tracking-wider"
                            >
                                {item}
                            </a>
                        ))}
                    </nav>
                    <button className="text-green-500 hover:text-zinc-300 text-xs tracking-wider" onClick={() => setIsOpen(true)}>
                        {'> JOIN_WAITLIST'}
                    </button>
